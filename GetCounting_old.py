...
import os
import imagej
from imagej import init

def GetCounting(ij,Folder_with_data, Type_of_stain,filename,scale):
  ...
  #  Create_folder(outputImages)
    ...
    #create new instance
    #ij = init(r'C:\\Fiji.app\\', headless=False)
...
saveAs("Results",outputDir + name_file + "SummaryCountingvs5.xls"); //intermediate result
...
run("Flatten");
saveAs("Tiff", outputImages + name_file + "All5.tif" );
close("*");
close("Results");
setBatchMode(false);
