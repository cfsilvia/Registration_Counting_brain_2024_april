...
def ManagerData(Folder_with_data, Type_of_stain):
    ...
    for index in range(len(X)):
        thread = threading.Thread(target=ManagerData_Second(X, X_file,index,Folder_with_data, Type_of_stain))
        jobs.append(thread)
...
def ManagerData_Second(X, X_file,index,Folder_with_data, Type_of_stain):
    #%%
  ...
  #  X_file = sorted(glob(Folder_with_data + Type_of_stain + '*.tif'))
   ...
   # for index in range(len(X)):
        ...
        #%%Get through image j countings of labels inside roi of atlas
         GetCounting(Folder_with_data, Type_of_stain,filename,ij)
