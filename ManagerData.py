...
from  FilterData import FilterData,  FilterDataOxytocin
    ...
    for index in range(len(X_file)):
        ...
        #read the image
         ...
         if Type_of_stain == "Cfos":
             ...
             coord_details = FilterDataOxytocin(labels,X, details)
         else:
             coord_details = details['coord']
             
        # #%% save the labels
         path =  Folder_with_data + 'Roi_all_image_'+ Type_of_stain + '/'
         Create_folder(path)
...
def ManagerCountingWithoutAtlas(Folder_with_data, Type_of_stain,scale):     
    Total_inputs = list()
    if Type_of_stain == "Cfos+Oxytocin":
        stains = Type_of_stain.split("+") #convert into a list
        X_file = sorted(glob(Folder_with_data + stains[0] + '/' +'*.tif')) #take only one color
    else:
        X_file = sorted(glob(Folder_with_data + Type_of_stain + '/' +'*.tif'))
    ...
    for index in range(len(X_file)):
        ...
        filename = os.path.splitext(file_name)[0]
        #%% Create list of inputs for future counting
        InputsString = CreateListInputsWithoutAtlas(Folder_with_data, Type_of_stain,scale,filename)
        #Create list for each filename
        Total_inputs.append(InputsString)  
    #%%Applied function to get counting in parallel
    GetCounting(Total_inputs,Type_of_stain)
...
def CreateListInputs(Folder_with_data, Type_of_stain,scale,filename):
    ...
    roi_input =  path =  Folder_with_data + 'Roi_all_image_'+ Type_of_stain + '/' + filename + '.zip'
...
def CreateListInputsWithoutAtlas(Folder_with_data, Type_of_stain,scale,filename):
    #%%#%% split 
    stains = Type_of_stain.split("+") #convert into a list
    #%%
    image_input = []
    for s in stains:
        image = Folder_with_data + s + '/' + filename + '.tif'
        image_input.append(image)
    #%%
    roi_input = list()
    for s in stains:
      roi =  path =  Folder_with_data + 'Roi_all_image_'+ s + '/' + filename + '.zip'
      roi_input.append(roi)
    #%%
    roi_atlas = Folder_with_data + 'ROI/' + 'RoiSet' + filename +'.zip'
    outputDir =  Folder_with_data + 'CountingResultOf' + Type_of_stain + '/'
    Create_folder(outputDir)
    outputImages = Folder_with_data + 'LabeledImagesOf' + Type_of_stain + '/'
    Create_folder(outputImages)
#%%Create list
    Inputs = [roi_input[0],roi_input[1], roi_atlas, outputDir, outputImages, image_input[0],image_input[1], Type_of_stain ,scale]
