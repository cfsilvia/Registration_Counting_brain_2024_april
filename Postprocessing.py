...
def SaveSector(Folder_with_data, Table_with_all_data,unique_sectors,separation):
    ...
    quantity_regions = len(unique_sectors)
    with pd.ExcelWriter(output) as writer:
      for s in unique_sectors:
        ...
        #consider cases in which the sector is nan- nothing with no data
        if len(data) > 0: #only regions with data
          index = s.find('/')
          if(index != -1):
            s =s.replace("/","_")
          ...
          quantity_regions = quantity_regions - 1 # remove nan no exist regions
      print("Number of regions",quantity_regions)  
